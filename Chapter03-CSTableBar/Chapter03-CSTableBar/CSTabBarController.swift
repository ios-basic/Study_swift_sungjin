...
class CSTabBarController: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        let width = self.view.frame.width // 뷰의 너비가 화면 전체 너비와 동일하도록 설정
        let height: CGFloat = 50 // 뷰의 높이를 50으로 설정
        let x: CGFloat = 0 // 뷰가 탭 바처럼 화면의 왼쪽에 빈 공간없이 채워지도록 x 좌표를 0으로 설정
        let y = self.view.frame.height - height // 뷰가 화면의 하단까지 빈 공간 없이 채워지도록 화면 전체 높이에서 뷰의 높이를 뺀 만큼을 y 좌표로 설정
